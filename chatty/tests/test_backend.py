...
def test_generate_label_filters_no_label():
    with patch.object(backend, 'llm') as mock_llm:
        mock_llm.invoke.return_value = MagicMock(content='[_NO_LABEL_]')
        assert backend.generate_label('hi') == ''

def test_generate_label_returns_text():
    with patch.object(backend, 'llm') as mock_llm:
        mock_llm.invoke.return_value = MagicMock(content='Trip Planning Discussion')
